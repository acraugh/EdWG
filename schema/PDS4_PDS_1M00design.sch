<?xml version="1.0" encoding="UTF-8"?>
  <!-- PDS4 Schematron for Name Space Id:pds  Version:1.22.0.0 - Thu Apr 18 10:55:19 EDT 2024 -->
...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
  ...
  <!-- *** BEGIN Added for Investigation Design work -->
  <sch:pattern>
    <sch:rule context="pds:reference_provides">
      <sch:assert test=". = ('Summary Description', 'Detailed Description', 'Calibration Procedure', 'Errata', 'Logo')">
        The pds:reference_provides value must be one of the defined permissible values: "Summary Description", "Detailed Description", "Calibration Procedure", "Errata", "Logo".
      </sch:assert>
    </sch:rule>
  </sch:pattern>
  
  <sch:pattern>
    <sch:rule context="pds:Associated_Name">
      <sch:assert test="pds:name_type = ('Name', 'Nickname', 'Acronym', 'Known Misspelling', 'Known Misnomer')">
        The name_type must be one of: 'Name', 'Nickname', 'Common Name', 'Acronym', 'Known Misspelling', 'Known Misnomer'. 
      </sch:assert>
      <sch:assert test="pds:name_status = ('Official', 'Informal', 'Preferred', 'Valid', 'Deprecated', 'Historic', 'Invalid')">
        The name_status must be oe of: 'Official', 'Informal', 'Preferred', 'Valid', 'Deprecated', 'Historic', 'Invalid'.
      </sch:assert>
    </sch:rule>
  </sch:pattern>
  <!-- *** END -->
  ...
  <sch:pattern>
    <sch:rule context="pds:Identification_Area/pds:information_model_version">
      <sch:assert test=". = ('1.22.0.0')">
        <title>pds:Identification_Area/pds:information_model_version/pds:information_model_version</title>
        The attribute pds:Identification_Area/pds:information_model_version must be equal to the value '1.22.0.0'.</sch:assert>
    </sch:rule>
  </sch:pattern>
  <sch:pattern>
  ...
  <!-- *** BEGIN Added for Investigation design testing -->
  <sch:pattern>
    <sch:rule context="pds:Investigation">
      <sch:assert test="if (pds:Operational_Phase)
        then (not(pds:no_phase_information_available) or (pds:no_phase_information_available eq 'false'))
        else true()">
        The attribute pds:no_phase_information_available should either be omitted or set to 'false' when one or more pds:Operational_Phase classes are present.
      </sch:assert>
      <sch:assert test="if (not(pds:Operational_Phase))
        then (pds:no_phase_information_available eq 'true')
        else true()">
        When no pds:Operational_Phase class is present, the pds:no_phase_information_available attribute must be present and set to 'true'.
      </sch:assert>
    </sch:rule>
  </sch:pattern>
  <!-- *** END -->
  ...
  <sch:pattern>
    <sch:rule context="pds:Investigation/pds:type">
      ...
      <!-- *** BEGIN Modified for Investigation design testing 
      ...
      <sch:assert test=". = ('Field Campaign', 'Individual Investigation', 'Primary Mission', 'Extended Mission', 'Observing Campaign', 'Other Investigation')">
        <title>pds:Investigation/pds:type/pds:type</title>
        The attribute pds:Investigation/pds:type must be equal to one of the following values 'Field Campaign', 'Individual Investigation', 'Primary Mission', 'Extended Mission', 'Observing Campaign', 'Other Investigation'.</sch:assert>
      <sch:assert test="if (data() eq 'Mission') then false() else true()">
        value: <sch:value-of select="data()"/>. 
        In the pds:Investigation context object, the pds:type value of "Mission" is deprecated. Instead, use either "Primary Mission" or "Extended Mission", as appropriate.
      </sch:assert>
      <!-- *** END -->
...
<!-- *** BEGIN
        Modifications for new reference_type values (in the 'or' clause). -->
      <sch:assert test="every $ref in (pds:reference_type) satisfies ($ref = ('context_to_associate', 'instrument_to_document', 'instrument_to_instrument_host', 'instrument_host_to_document', 'instrument_host_to_instrument', 'instrument_host_to_investigation', 'instrument_host_to_target', 'investigation_to_document', 'investigation_to_instrument', 'investigation_to_instrument_host', 'investigation_to_target', 'node_to_personnel', 'node_to_agency', 'node_to_manager', 'node_to_operator', 'node_to_data_archivist', 'resource_to_instrument', 'resource_to_instrument_host', 'resource_to_investigation', 'resource_to_target', 'target_to_document', 'target_to_instrument', 'target_to_instrument_host', 'target_to_investigation', 'instrument_to_telescope', 'instrument_to_observatory', 'instrument_to_facility', 'telescope_to_observatory', 'telescope_to_facility', 'telescope_to_airborne', 'investigation_to_telescope', 'facility_to_investigation', 'facility_to_instrument', 'facility_to_telescope', 'investigation_to_facility', 'telescope_to_instrument')) 
                                                                  or ($ref = ('investigation_to_agency'))">
